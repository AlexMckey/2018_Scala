...
object Sol_15_NotMy {
  ...
  def simulateCombat(grid: Grid[Char], units: List[CombatUnit], elfDeath: Boolean = false): (Int, List[CombatUnit]) = {
    ...
    val roundIt = Iterator.iterate((units, false))({ case (u, _) => round(u) })
  ...
  lazy val input: String = io.Source.fromInputStream(getClass.getResourceAsStream("input15.txt")).mkString.stripLineEnd
